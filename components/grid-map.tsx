  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Delhi Power Grid - Interactive Map</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Interactive visualization showing power generation facilities, substations, and transmission lines in Delhi NCR
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <Button
                  variant={showSubstations ? "default" : "outline"}
                  size="sm"
                  onClick={() => setShowSubstations(!showSubstations)}
                  className="h-8 px-3"
                >
                  {showSubstations ? <Eye className="w-4 h-4 mr-1" /> : <EyeOff className="w-4 h-4 mr-1" />}
                  Subs
                </Button>
              </div>
            </CardTitle>
        ...
        {/* Power Houses Info Panel */}
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Zap className="w-5 h-5" />
              Power Houses ({powerHouses.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3 max-h-80 overflow-y-auto">
              {powerHouses.map((powerHouse) => (
                <div
                  ...
                  onClick={() => setSelectedPowerHouse(selectedPowerHouse === powerHouse.id ? null : powerHouse.id)}
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium text-sm text-gray-900 dark:text-white flex items-center gap-2">
                      <span className="text-lg">{powerHouse.type.charAt(0)}</span>
                      {powerHouse.name}
                    </span>
                  ...
                  <div className="text-xs text-gray-700 dark:text-gray-300">
                    Type: {powerHouse.type} | Capacity: {powerHouse.capacity}
                  </div>
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-xs text-gray-600 dark:text-gray-400">
                      {powerHouse.status === "active"
                        ? "Operational"
                        : powerHouse.status === "maintenance"
                          ? "Under Maintenance"
                          : "Offline"}
                    </span>
                  </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card>
          <CardContent className="pt-6">
            <h3 className="font-semibold mb-4 text-gray-900 dark:text-white">Substations ({substations.length})</h3>
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-purple-600 border-2 border-white" />
                <span className="text-sm text-gray-700 dark:text-gray-300">
                  Primary (400kV) - {substations.filter((s) => s.type === "primary").length}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-blue-600 border-2 border-white rounded-sm" />
                <span className="text-sm text-gray-700 dark:text-gray-300">
                  Secondary (132kV) - {substations.filter((s) => s.type === "secondary").length}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-green-600 border-2 border-white rounded-full" />
                <span className="text-sm text-gray-700 dark:text-gray-300">
                  Distribution (11-33kV) - {substations.filter((s) => s.type === "distribution").length}
                </span>
              </div>
            </div>
          </CardContent>
        ...
        <Card>
          <CardContent className="pt-6">
            <h3 className="font-semibold mb-4 text-gray-900 dark:text-white">Power Plants ({powerHouses.length})</h3>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-700 dark:text-gray-300">🔥 Thermal:</span>
                <span className="font-medium text-gray-900 dark:text-white">
                  {powerHouses.filter((p) => p.type === "thermal").length}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-700 dark:text-gray-300">⚡ Gas:</span>
                <span className="font-medium text-gray-900 dark:text-white">
                  {powerHouses.filter((p) => p.type === "gas").length}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-700 dark:text-gray-300">☀️ Solar:</span>
                <span className="font-medium text-gray-900 dark:text-white">
                  {powerHouses.filter((p) => p.type === "solar").length}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-700 dark:text-gray-300">💧 Hydro:</span>
                <span className="font-medium text-gray-900 dark:text-white">
                  {powerHouses.filter((p) => p.type === "hydro").length}
                </span>
              </div>
              <div className="border-t pt-2 mt-2">
